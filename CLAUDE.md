...
- Image gen: ComfyUI API (localhost:8188) — mocked locally via COMFYUI_MOCK=true
- Brand memory: ChromaDB (localhost:8000)
- Data store: SQLite via SQLAlchemy (async with aiosqlite)
- All services communicate via localhost within Docker Compose (production) or hybrid setup (local dev)
...
1. **Docker Compose** — get ChromaDB running (Ollama runs natively on macOS)
2. **Backend skeleton** — FastAPI app, SQLite models, health check endpoints
3. **Pipeline steps** — brief parser → brand memory → prompt engineer → image gen → compositor → compliance checker (test each individually)
...
10. **Deploy** — docker-compose.yml for Vast.ai GPU cloud

---

## Local Dev Environment

- **Machine:** MacBook Pro M2 Pro, 16GB RAM
- **Ollama:** Runs natively (not Docker) with Metal acceleration
- **LLM Model:** Qwen 3 4B locally, Gemma 4 26B MoE on Vast.ai
- **Vision Model:** LLaVA 7B locally, Gemma 4 26B MoE on Vast.ai
- **ComfyUI:** Mocked locally (COMFYUI_MOCK=true), real on Vast.ai
- **ChromaDB:** Docker container via docker-compose.dev.yml
